...
class ImageUploadComponent extends Object
{
	...
	*/	
	function upload_image_and_thumbnail($data, $datakey, $imgscale, $thumbscale, $folderName, $square) {
		...
		if (strlen($data['images']['filedata']['name'])>4){ 
					$error = 0;
					...
					$filetype = $this->getFileExtension($data['images']['filedata']['name']);
					...
					{
						// Get the image size
						$imgsize = GetImageSize($data['images']['filedata']['tmp_name']);
					}
					...
					if (is_uploaded_file($data['images']['filedata']['tmp_name']))
                    {                    
						// Copy the image into the temporary directory
                        if (!copy($data['images']['filedata']['tmp_name'],"$tempfile"))
                        {
